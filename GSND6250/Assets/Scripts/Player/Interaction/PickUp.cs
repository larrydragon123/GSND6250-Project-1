...
public class PickUp : MonoBehaviour
{
    ...
    void Start()
    {
        HoldLayer = LayerMask.NameToLayer("HoldLayer"); //if your holdLayer is named differently make sure to change this ""
        PickUpLayer = LayerMask.NameToLayer("PickUpLayer"); //if your holdLayer is named differently make sure to change this ""
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.E)) //change E to whichever key you want to press to pick up
        {
            if (heldObj == null) //if currently not holding anything
            {
                ...
                RaycastHit hit;
                if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, pickUpRange, LayerMask.GetMask("PickUpLayer")))
                {
